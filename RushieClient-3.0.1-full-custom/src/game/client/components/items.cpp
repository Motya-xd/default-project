...
void CItems::RenderLaser(vec2 From, vec2 Pos, ColorRGBA OuterColor, ColorRGBA InnerColor, float TicksBody, float TicksHead, int Type, float GlowIntensity) const
{
	...
	if(g_Config.m_RiBetterLasers)
	{
		if(Len > 0)
		{
			...
			const int Effect = Type == LASERTYPE_SHOTGUN ? g_Config.m_RcShotgunPulse : Type == LASERTYPE_RIFLE ? g_Config.m_RcLaserPulse : 0;
			float EffectThickness = 1.0f;
		if(Type == LASERTYPE_SHOTGUN)
		{
			EffectThickness = (Effect == 1 ? g_Config.m_RcShotgunPulseThickness :
				Effect == 2 ? g_Config.m_RcShotgunPrismaticThickness :
				Effect == 3 ? g_Config.m_RcShotgunCrystalGlowThickness :
				Effect == 4 ? g_Config.m_RcShotgunEnchantedGlowThickness : 100) /
				100.0f;
		}
		else if(Type == LASERTYPE_RIFLE)
		{
			EffectThickness = (Effect == 1 ? g_Config.m_RcLaserPulseThickness :
				Effect == 2 ? g_Config.m_RcLaserPrismaticThickness :
				Effect == 3 ? g_Config.m_RcLaserCrystalGlowThickness :
				Effect == 4 ? g_Config.m_RcLaserEnchantedGlowThickness : 100) /
				100.0f;
		}
		const float EffectTime = Client()->LocalTime();
		const vec2 Perpendicular(Dir.y, -Dir.x);

		if(Effect != 0 && GlowIntensity == 0.0f)
		{
		Graphics()->TextureClear();
		Graphics()->QuadsBegin();
		auto DrawBeam = [&](vec2 BeamFrom, vec2 BeamTo, float Width, ColorRGBA Color) {
			const vec2 Offset = Perpendicular * Width;
			const IGraphics::CFreeformItem Beam(BeamFrom - Offset, BeamFrom + Offset, BeamTo - Offset, BeamTo + Offset);
			Graphics()->SetColor(Color);
			Graphics()->QuadsDrawFreeform(&Beam, 1);
		};
		auto DrawSegment = [&](vec2 SegmentFrom, vec2 SegmentTo, float Width, ColorRGBA Color) {
			const vec2 SegmentDelta = SegmentTo - SegmentFrom;
			const float SegmentLength = length(SegmentDelta);
			if(SegmentLength <= 0.0f)
				return;
			const vec2 SegmentDir = SegmentDelta / SegmentLength;
			const vec2 SegmentOffset(SegmentDir.y * Width, -SegmentDir.x * Width);
			const IGraphics::CFreeformItem Segment(SegmentFrom - SegmentOffset, SegmentFrom + SegmentOffset, SegmentTo - SegmentOffset, SegmentTo + SegmentOffset);
			Graphics()->SetColor(Color);
			Graphics()->QuadsDrawFreeform(&Segment, 1);
		};
		auto DrawRing = [&](vec2 Center, float Radius, float Width, ColorRGBA Color) {
			constexpr int RingSegments = 20;
			for(int i = 0; i < RingSegments; ++i)
			{
				const float Angle0 = 2.0f * pi * i / RingSegments;
				const float Angle1 = 2.0f * pi * (i + 1) / RingSegments;
				const vec2 RingFrom = Center + direction(Angle0) * Radius;
				const vec2 RingTo = Center + direction(Angle1) * Radius;
				const vec2 RingDir = normalize(RingTo - RingFrom);
				const vec2 RingOffset(RingDir.y * Width, -RingDir.x * Width);
				const IGraphics::CFreeformItem Segment(RingFrom - RingOffset, RingFrom + RingOffset, RingTo - RingOffset, RingTo + RingOffset);
				Graphics()->SetColor(Color);
				Graphics()->QuadsDrawFreeform(&Segment, 1);
			}
		};

		auto DrawPulseSpiral = [&](bool Front) {
			constexpr int SpiralSegments = 48;
			const float SpiralScale = maximum(Ia, 0.25f);
			const float Turns = maximum(Len / 64.0f, 1.0f);
			const float PhaseOffset = EffectTime * 5.0f;
			for(int i = 0; i < SpiralSegments; ++i)
			{
				const float Start = i / static_cast<float>(SpiralSegments);
				const float End = (i + 1) / static_cast<float>(SpiralSegments);
				const float StartPhase = Start * Turns * 2.0f * pi - PhaseOffset;
				const float EndPhase = End * Turns * 2.0f * pi - PhaseOffset;
				const float MiddlePhase = (StartPhase + EndPhase) * 0.5f;
				if((std::cos(MiddlePhase) >= 0.0f) != Front)
					continue;
				const vec2 SpiralFrom = mix(From, Pos, Start) + Perpendicular * std::sin(StartPhase) * 9.0f * SpiralScale;
				const vec2 SpiralTo = mix(From, Pos, End) + Perpendicular * std::sin(EndPhase) * 9.0f * SpiralScale;
				const float DepthAlpha = 0.55f + 0.45f * absolute(std::cos(MiddlePhase));
				DrawSegment(SpiralFrom, SpiralTo, 1.2f * EffectThickness * SpiralScale, InnerColor.WithMultipliedAlpha(DepthAlpha));
			}
		};

		if(Effect == 1) // Pulse Spiral
		{
			DrawBeam(From, Pos, 10.0f * Ia * EffectThickness, OuterColor.WithMultipliedAlpha(0.16f));
			DrawPulseSpiral(false);
		}
		else if(Effect == 2) // Prismatic
		{
			for(int i = 0; i < 12; ++i)
			{
				const float Start = i / 12.0f;
				const float End = (i + 1) / 12.0f;
				const float Hue = std::fmod(EffectTime * 0.18f + Start, 1.0f);
				DrawBeam(mix(From, Pos, Start), mix(From, Pos, End), 7.5f * Ia * EffectThickness, color_cast<ColorRGBA>(ColorHSLA(Hue, 1.0f, 0.55f, InnerColor.a)));
			}
			const float Shift = std::fmod(EffectTime * 0.8f, 0.2f);
			for(float Progress = Shift; Progress < 1.0f; Progress += 0.2f)
			{
				const vec2 Center = mix(From, Pos, Progress);
				const ColorRGBA PrismColor = color_cast<ColorRGBA>(ColorHSLA(std::fmod(EffectTime * 0.2f + Progress, 1.0f), 1.0f, 0.6f, InnerColor.a));
				DrawBeam(Center - Perpendicular * 11.0f * Ia, Center + Perpendicular * 11.0f * Ia, 1.0f * EffectThickness, PrismColor);
				DrawRing(Center, 9.0f * Ia, 0.8f * EffectThickness, PrismColor.WithMultipliedAlpha(0.8f));
			}
			DrawRing(Pos, 10.0f * Ia, 1.2f * EffectThickness, color_cast<ColorRGBA>(ColorHSLA(std::fmod(EffectTime * 0.2f, 1.0f), 1.0f, 0.6f, InnerColor.a)));
		}
		else if(Effect == 3) // Crystal Glow
		{
			const float Pulse = 0.8f + 0.2f * std::sin(EffectTime * pi * 3.0f);
			DrawBeam(From, Pos, 16.0f * Ia * Pulse * EffectThickness, OuterColor.WithMultipliedAlpha(0.18f));
			DrawBeam(From, Pos, 11.0f * Ia * EffectThickness, InnerColor.WithMultipliedAlpha(0.48f));
			DrawBeam(From, Pos, 3.0f * Ia * EffectThickness, ColorRGBA(1.0f, 1.0f, 1.0f, InnerColor.a * 0.9f));
			const float ParticleShift = std::fmod(EffectTime * 0.55f, 0.12f);
			int ParticleIndex = 0;
			for(float Progress = ParticleShift; Progress < 1.0f; Progress += 0.12f, ++ParticleIndex)
			{
				const vec2 Center = mix(From, Pos, Progress);
				const float Side = (ParticleIndex & 1) ? 1.0f : -1.0f;
				const float Spark = 10.0f + 5.0f * std::sin(EffectTime * 5.0f + ParticleIndex);
				DrawBeam(Center + Perpendicular * 6.0f * Ia * Side, Center + Perpendicular * Spark * Ia * Side + Dir * 5.0f, 1.2f * EffectThickness, InnerColor.WithMultipliedAlpha(0.9f));
				DrawRing(Center + Perpendicular * Spark * Ia * Side, 2.0f + Pulse, 0.65f * EffectThickness, ColorRGBA(1.0f, 1.0f, 1.0f, InnerColor.a * 0.75f));
			}
			const float RingScale = maximum(Ia, 0.25f);
			DrawRing(Pos, (13.0f + 2.0f * std::sin(EffectTime * 2.0f)) * RingScale, 1.5f * EffectThickness, OuterColor);
			DrawRing(Pos, (8.0f + 2.0f * std::sin(EffectTime * 3.5f + 1.0f)) * RingScale, 1.2f * EffectThickness, InnerColor);
			DrawRing(Pos, (4.0f + std::sin(EffectTime * 5.0f)) * RingScale, 0.9f * EffectThickness, ColorRGBA(1.0f, 1.0f, 1.0f, InnerColor.a));
		}
		else if(Effect == 4) // Enchanted Glow, an original multi-pass DLS-inspired style
		{
			const int PowerValue = Type == LASERTYPE_SHOTGUN ? g_Config.m_RcShotgunEnchantedGlowPower : g_Config.m_RcLaserEnchantedGlowPower;
			const float Power = PowerValue / 100.0f;
			const float Pulse = 0.82f + 0.18f * std::sin(EffectTime * (2.5f + Power * 2.0f));
			const float Fade = maximum(Ia, 0.18f);
			DrawBeam(From, Pos, (12.0f + 14.0f * Power) * Fade * Pulse * EffectThickness, OuterColor.WithMultipliedAlpha(0.05f + 0.11f * Power));
			DrawBeam(From, Pos, (8.0f + 8.0f * Power) * Fade * EffectThickness, InnerColor.WithMultipliedAlpha(0.1f + 0.2f * Power));
			DrawBeam(From, Pos, (2.0f + 1.5f * Power) * Fade * EffectThickness, ColorRGBA(1.0f, 1.0f, 1.0f, InnerColor.a * (0.5f + 0.4f * Power)));

			const float SparkStep = 0.16f - 0.06f * Power;
			const float SparkShift = std::fmod(EffectTime * (0.25f + 0.35f * Power), SparkStep);
			int SparkIndex = 0;
			for(float Progress = SparkShift; Progress < 1.0f; Progress += SparkStep, ++SparkIndex)
			{
				const vec2 Center = mix(From, Pos, Progress);
				const float SparkPulse = 0.65f + 0.35f * std::sin(EffectTime * 5.0f + SparkIndex * 1.7f);
				const float SparkLength = (4.0f + 7.0f * Power) * Fade * SparkPulse;
				DrawBeam(Center - Perpendicular * SparkLength, Center + Perpendicular * SparkLength, (0.55f + 0.45f * Power) * EffectThickness, InnerColor.WithMultipliedAlpha(0.35f + 0.45f * Power));
			}

			const float HeadRadius = (8.0f + 8.0f * Power) * Fade * Pulse;
			DrawRing(Pos, HeadRadius, (0.8f + Power) * EffectThickness, OuterColor.WithMultipliedAlpha(0.45f + 0.35f * Power));
			DrawRing(Pos, HeadRadius * 0.58f, (0.65f + 0.7f * Power) * EffectThickness, InnerColor);
		}

		// do outline
		Graphics()->SetColor(OuterColor);
		vec2 Out = vec2(Dir.y, -Dir.x) * (7.0f * Ia * EffectThickness);
		...
		// do inner
		Out = vec2(Dir.y, -Dir.x) * (5.0f * Ia * EffectThickness);
