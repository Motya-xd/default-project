...
void CControls::ConDummyLeft(IConsole::IResult *pResult, void *pUserData)
{
	static_cast<CControls *>(pUserData)->SetDummyDirection(pResult, -1);
}

void CControls::ConDummyRight(IConsole::IResult *pResult, void *pUserData)
{
	static_cast<CControls *>(pUserData)->SetDummyDirection(pResult, 1);
}
...
{
	CControls *pControls = static_cast<CControls *>(pUserData);
	pControls->m_DummyJump = pResult->GetInteger(0) != 0;
	pControls->GameClient()->m_DummyInput.m_Jump = pControls->m_DummyJump;
}

void CControls::ConDummyHammer(IConsole::IResult *pResult, void *pUserData)
{
	CControls *pControls = static_cast<CControls *>(pUserData);
	CNetObj_PlayerInput &DummyInput = pControls->GameClient()->m_DummyInput;
	pControls->m_DummyHammer = pResult->GetInteger(0) != 0;

	if(pControls->m_DummyHammer)
	{
		DummyInput.m_WantedWeapon = WEAPON_HAMMER + 1;
		if((DummyInput.m_Fire & 1) == 0)
			DummyInput.m_Fire++;
	}
	else if((DummyInput.m_Fire & 1) != 0)
	{
		DummyInput.m_Fire++;
	}
}

void CControls::ConDummyCopy(IConsole::IResult *pResult, void *pUserData)
{
	CControls *pControls = static_cast<CControls *>(pUserData);
	pControls->m_DummyCopyNoHook = !pControls->m_DummyCopyNoHook;
	if(pControls->m_DummyCopyNoHook && g_Config.m_ClDummyCopyMoves)
		pControls->GameClient()->m_DummyInput.m_Hook = 0;
}
...
void CControls::OnConsoleInit()
{
	...
	Console()->Register("+dummy_left", "", CFGFLAG_CLIENT, ConDummyLeft, this, "Move the inactive tee left while held");
	Console()->Register("+dummy_right", "", CFGFLAG_CLIENT, ConDummyRight, this, "Move the inactive tee right while held");
	Console()->Register("+dummy_jump", "", CFGFLAG_CLIENT, ConDummyJump, this, "Jump with the inactive tee while held");
	Console()->Register("+dummy_hammer", "", CFGFLAG_CLIENT, ConDummyHammer, this, "Hit with the inactive tee's hammer while held");
	Console()->Register("dummy_copy", "", CFGFLAG_CLIENT, ConDummyCopy, this, "Toggle copying moves to the inactive tee without hook");
	{
