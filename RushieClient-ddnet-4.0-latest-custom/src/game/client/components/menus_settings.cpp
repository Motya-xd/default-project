...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	static CScrollRegion s_SettingsTabScrollRegion;
	vec2 TabScrollOffset(0.0f, 0.0f);
	CScrollRegionParams TabScrollParams;
	TabScrollParams.m_ScrollUnit = 36.0f;
	TabScrollParams.m_ScrollbarWidth = 8.0f;
	TabScrollParams.m_ScrollbarMargin = 2.0f;
	s_SettingsTabScrollRegion.Begin(&TabBar, &TabScrollOffset, &TabScrollParams);
	CUIRect TabContent = TabBar;
	TabContent.y += TabScrollOffset.y;

	for(int i = 0; i < SETTINGS_LENGTH; i++)
	{
		if(i == SETTINGS_MY_BINDS && !g_Config.m_RcShowMyBindsTab)
			continue;
		TabContent.HSplitTop(10.0f, nullptr, &TabContent);
		TabContent.HSplitTop(26.0f, &Button, &TabContent);
		if(s_SettingsTabScrollRegion.AddRect(Button) && DoButton_MenuTab(&s_aTabButtons[i], apTabs[i], g_Config.m_UiSettingsPage == i, &Button, IGraphics::CORNER_R, &m_aAnimatorsSettingsTab[i]))
			g_Config.m_UiSettingsPage = i;
	}
	s_SettingsTabScrollRegion.End();
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_MY_BINDS)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_CONTROLS);
		m_MenusSettingsControls.RenderMyBinds(MainView);
	}
	else
	{
