...
void CHud::RenderDummyActions()
{
	...
	float x = StartX + 2;
	Graphics()->SetColor(1.0f, 1.0f, 1.0f, 0.4f);
	if(g_Config.m_ClDummyHammer || GameClient()->m_Controls.m_DummyHammer)
	{
		Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
	}
	...
	y += 13;
	Graphics()->SetColor(1.0f, 1.0f, 1.0f, 0.4f);
	if(g_Config.m_ClDummyCopyMoves || g_Config.m_ClDummyDirection != 0 || GameClient()->m_Controls.m_DummyJump)
	{
		Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
	}
...
void CHud::RenderDummyCopyNoHook()
{
	if(!g_Config.m_RcDummyCopyHud || !GameClient()->m_Controls.m_DummyCopyNoHook || !g_Config.m_ClDummyCopyMoves)
		return;

	const float Scale = g_Config.m_RcDummyCopyHudSize / 100.0f;
	const float FontSize = 7.0f * Scale;
	const float CardWidth = 76.0f * Scale;
	const float CardHeight = 16.0f * Scale;
	const float HorizontalRange = maximum(0.0f, m_Width - CardWidth - 8.0f);
	const float HorizontalOffset = HorizontalRange * g_Config.m_RcDummyCopyHudX / 100.0f;
	const float X = g_Config.m_RcDummyCopyHudSide ? m_Width - CardWidth - 4.0f - HorizontalOffset : 4.0f + HorizontalOffset;
	const float Y = std::clamp(m_Height * g_Config.m_RcDummyCopyHudY / 100.0f - CardHeight / 2.0f, 4.0f, m_Height - CardHeight - 4.0f);
	const ColorRGBA ActiveColor = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_RcDummyCopyHudActiveColor));

	Graphics()->TextureClear();
	CUIRect Card = {X, Y, CardWidth, CardHeight};
	Card.Draw(ColorRGBA(0.02f, 0.02f, 0.025f, 0.82f), IGraphics::CORNER_ALL, 2.0f * Scale);

	TextRender()->TextColor(ActiveColor);
	TextRender()->Text(X + 5.0f * Scale, Y + 2.0f * Scale, FontSize, "COPY / NO HOOK");
	TextRender()->TextColor(TextRender()->DefaultTextColor());
}

void CHud::RenderTower()
{
	if(!g_Config.m_RcTowerHud || !Client()->DummyConnected())
		return;

	const int LocalId = GameClient()->m_aLocalIds[g_Config.m_ClDummy];
	const int DummyId = GameClient()->m_aLocalIds[!g_Config.m_ClDummy];
	if(!in_range(LocalId, MAX_CLIENTS - 1) || !in_range(DummyId, MAX_CLIENTS - 1) ||
		!GameClient()->m_Snap.m_aCharacters[LocalId].m_Active || !GameClient()->m_Snap.m_aCharacters[DummyId].m_Active)
		return;

	const CNetObj_Character &LocalChar = GameClient()->m_Snap.m_aCharacters[LocalId].m_Cur;
	const CNetObj_Character &DummyChar = GameClient()->m_Snap.m_aCharacters[DummyId].m_Cur;
	if(LocalChar.m_X != DummyChar.m_X || absolute(LocalChar.m_Y - DummyChar.m_Y) < 8)
		return;

	const float Scale = g_Config.m_RcTowerHudSize / 100.0f;
	const float FontSize = 7.0f * Scale;
	const float CardWidth = 38.0f * Scale;
	const float CardHeight = 16.0f * Scale;
	const float X = std::clamp(m_Width * g_Config.m_RcTowerHudX / 100.0f - CardWidth / 2.0f, 4.0f, m_Width - CardWidth - 4.0f);
	const float Y = std::clamp(m_Height * g_Config.m_RcTowerHudY / 100.0f - CardHeight / 2.0f, 4.0f, m_Height - CardHeight - 4.0f);
	const ColorRGBA TowerColor = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_RcTowerHudColor));

	Graphics()->TextureClear();
	CUIRect Card = {X, Y, CardWidth, CardHeight};
	Card.Draw(ColorRGBA(0.02f, 0.02f, 0.025f, 0.82f), IGraphics::CORNER_ALL, 2.0f * Scale);

	TextRender()->TextColor(TowerColor);
	TextRender()->Text(X + 5.0f * Scale, Y + 2.0f * Scale, FontSize, "TOWER");
	TextRender()->TextColor(TextRender()->DefaultTextColor());
}
