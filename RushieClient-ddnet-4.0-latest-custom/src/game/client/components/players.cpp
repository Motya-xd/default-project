...
float CPlayers::GetPlayerTargetAngle(
	...
	float Intra)
{
	if(Client()->DummyConnected() && GameClient()->m_aLocalIds[!g_Config.m_ClDummy] == ClientId)
	{
	...
	int ClientId)
{
	...
	const bool OwnConnectionTee = Local || (Client()->DummyConnected() && GameClient()->m_aLocalIds[!g_Config.m_ClDummy] == ClientId);

#if defined(CONF_VIDEORECORDER)
	if(IVideo::Current() && !g_Config.m_ClVideoShowHookCollOther && !OwnConnectionTee)
		return;
#endif
	...
	const int ShowMode = OwnConnectionTee ? g_Config.m_ClShowHookCollOwn : g_Config.m_ClShowHookCollOther;
	const bool HookCollRequested = OwnConnectionTee && Client()->State() != IClient::STATE_DEMOPLAYBACK ?
		GameClient()->m_Controls.m_aShowHookColl[g_Config.m_ClDummy] != 0 : Aim;
	const bool AlwaysRenderHookColl = ShowMode == 2;
	const bool RenderHookCollPlayer = ShowMode > 0 && HookCollRequested;
	...
	if(OwnConnectionTee && Client()->State() != IClient::STATE_DEMOPLAYBACK)
	{
		const CNetObj_PlayerInput &Input = Local ? GameClient()->m_Controls.m_aInputData[g_Config.m_ClDummy] : GameClient()->m_DummyInput;
		Angle = angle(vec2(Input.m_TargetX, Input.m_TargetY));
	}
	...
	CCharacter *pPredictedOwner = GameClient()->m_PredictedWorld.GetCharacterById(ClientId);
	const int TuneZone = Collision()->IsTune(Collision()->GetMapIndex(Position));
	const CTuningParams *pTuning = pPredictedOwner ? &pPredictedOwner->Core()->m_Tuning : GameClient()->GetTuning(TuneZone);
	float HookLength = (float)pTuning->m_HookLength;
	float HookFireSpeed = (float)pTuning->m_HookFireSpeed;
	const bool CanHookPlayers = pTuning->m_PlayerHooking && (!pPredictedOwner || !pPredictedOwner->Core()->m_HookHitDisabled);
	...
	auto IntersectPredictedCharacter = [&](const vec2 &From, const vec2 &To, vec2 &HitPos, vec2 &PlayerPos) {
		if(!CanHookPlayers)
			return false;
		if(pPredictedOwner)
		{
			CCharacter *pHitCharacter = GameClient()->m_PredictedWorld.IntersectCharacter(From, To, 2.0f, HitPos, pPredictedOwner, ClientId);
			if(pHitCharacter)
			{
				PlayerPos = pHitCharacter->GetPos();
				return true;
			}
			return false;
		}
		return GameClient()->IntersectCharacter(From, To, HitPos, ClientId, &PlayerPos) != -1;
	};
	...
	for(HookTick = 0; HookTick < MaxHookTicks; ++HookTick)
	{
		...
		if(distance(BasePos, SegmentEndPos) > HookLength)
		{
			...
			if(!HookEnteredTelehook)
			{
				...
				if(IntersectPredictedCharacter(SegmentStartPos, HitPos, RetractingHookEndPos, IntersectedPlayerPosition))
				{
		...
		if(IntersectPredictedCharacter(SegmentStartPos, HitPos, SegmentEndPos, IntersectedPlayerPosition))
		{
	...
	const int HookCollSize = OwnConnectionTee ? g_Config.m_ClHookCollSize : g_Config.m_ClHookCollSizeOther;
