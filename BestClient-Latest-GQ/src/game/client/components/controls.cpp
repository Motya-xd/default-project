...
struct CInputState
{
	...
	int m_ActiveDummy = -1;
	bool m_ReleaseOnCurrentDummy = false;
};
...
void CControls::ConKeyInputState(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		pState->m_ActiveDummy = g_Config.m_ClDummy;
		*pState->m_apVariables[pState->m_ActiveDummy] = 1;
	}
	else
	{
		// Release the key for the tee on which it was originally pressed.
		// Otherwise switching main/dummy while holding a direction leaves the
		// original tee moving because the release event targets the new tee.
		const int Dummy = pState->m_ReleaseOnCurrentDummy || pState->m_ActiveDummy < 0 ? g_Config.m_ClDummy : pState->m_ActiveDummy;
		*pState->m_apVariables[Dummy] = 0;
		pState->m_ActiveDummy = -1;
	}
}
...
void CControls::SetDummyDirection(IConsole::IResult *pResult, int Direction)
{
	if(pResult->GetInteger(0))
		g_Config.m_ClDummyDirection = Direction;
	else if(g_Config.m_ClDummyDirection == Direction)
		g_Config.m_ClDummyDirection = 0;
	GameClient()->m_DummyInput.m_Direction = g_Config.m_ClDummyDirection;
}
void CControls::ConDummyLeft(IConsole::IResult *pResult, void *pUserData) { static_cast<CControls *>(pUserData)->SetDummyDirection(pResult, -1); }
void CControls::ConDummyRight(IConsole::IResult *pResult, void *pUserData) { static_cast<CControls *>(pUserData)->SetDummyDirection(pResult, 1); }
void CControls::ConDummyJump(IConsole::IResult *pResult, void *pUserData)
{
	auto *pThis = static_cast<CControls *>(pUserData);
	pThis->m_DummyJump = pResult->GetInteger(0) != 0;
	pThis->GameClient()->m_DummyInput.m_Jump = pThis->m_DummyJump;
}
void CControls::ConDummyHammer(IConsole::IResult *pResult, void *pUserData)
{
	auto *pThis = static_cast<CControls *>(pUserData);
	auto &Input = pThis->GameClient()->m_DummyInput;
	pThis->m_DummyHammer = pResult->GetInteger(0) != 0;
	if(pThis->m_DummyHammer)
	{
		Input.m_WantedWeapon = WEAPON_HAMMER + 1;
		if((Input.m_Fire & 1) == 0) Input.m_Fire++;
	}
	else if((Input.m_Fire & 1) != 0) Input.m_Fire++;
}
void CControls::ConDummyCopy(IConsole::IResult *, void *pUserData)
{
	auto *pThis = static_cast<CControls *>(pUserData);
	pThis->m_DummyCopyNoHook = !pThis->m_DummyCopyNoHook;
	if(pThis->m_DummyCopyNoHook) pThis->GameClient()->m_DummyInput.m_Hook = 0;
}
...
void CControls::OnConsoleInit()
{
	...
	{
		static CInputState s_State = {this, {&m_aInputData[0].m_Hook, &m_aInputData[1].m_Hook}, -1, true};
	...
	Console()->Register("+dummy_left", "", CFGFLAG_CLIENT, ConDummyLeft, this, "Move inactive tee left");
	Console()->Register("+dummy_right", "", CFGFLAG_CLIENT, ConDummyRight, this, "Move inactive tee right");
	Console()->Register("+dummy_jump", "", CFGFLAG_CLIENT, ConDummyJump, this, "Jump with inactive tee");
	Console()->Register("+dummy_hammer", "", CFGFLAG_CLIENT, ConDummyHammer, this, "Hammer with inactive tee");
	Console()->Register("dummy_copy", "", CFGFLAG_CLIENT, ConDummyCopy, this, "Toggle dummy copy without hook");
	{
	...
	else
	{
		...
		if(g_Config.m_ClDummyCopyMoves)
		{
			...
			if(!GameClient()->m_Snap.m_SpecInfo.m_Active || GameClient()->m_Snap.m_SpecInfo.m_SpectatorId < 0)
			{
				...
				pDummyInput->m_Hook = m_DummyCopyNoHook ? 0 : m_aInputData[g_Config.m_ClDummy].m_Hook;
		...
		if(g_Config.m_ClDummyControl)
		{
			...
			if(m_DummyHammer)
			{
				...
				if((pDummyInput->m_Fire & 1) == 0) pDummyInput->m_Fire++;
			}
			else if(g_Config.m_ClDummyFire)
		...
		if(g_Config.m_ClDummyDirection != 0)
			GameClient()->m_DummyInput.m_Direction = g_Config.m_ClDummyDirection;
		if(g_Config.m_ClDummyCopyMoves && m_DummyCopyNoHook)
			GameClient()->m_DummyInput.m_Hook = 0;
		if(m_DummyJump)
			GameClient()->m_DummyInput.m_Jump = 1;
