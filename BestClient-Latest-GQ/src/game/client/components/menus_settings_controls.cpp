...
void CMenusSettingsControls::OnInterfacesInit(CGameClient *pClient)
{
	...
	m_vBindOptions = {
		...
		{EBindOptionGroup::GQ, Localizable("Dummy left"), "+dummy_left"},
		{EBindOptionGroup::GQ, Localizable("Dummy right"), "+dummy_right"},
		{EBindOptionGroup::GQ, Localizable("Dummy jump"), "+dummy_jump"},
		{EBindOptionGroup::GQ, Localizable("Dummy hammer"), "+dummy_hammer"},
		{EBindOptionGroup::GQ, Localizable("Dummy copy without hook"), "dummy_copy"},
...
void CMenusSettingsControls::RenderGq(CUIRect MainView)
{
	UpdateBindOptions();
	const CUIRect Background = MainView;
	if(g_Config.m_BcGqAnimatedBackground)
	{
		constexpr int MaxPoints = 28;
		vec2 aPoints[MaxPoints];
		IGraphics::CQuadItem aDots[MaxPoints];
		IGraphics::CLineItem aLines[MaxPoints * MaxPoints / 2];
		const int Count = g_Config.m_BcGqBackgroundTriangleCount;
		const float Time = std::fmod(Client()->GlobalTime(), 4096.0f) * g_Config.m_BcGqBackgroundCubeSpeed / 100.0f;
		int NumLines = 0;
		for(int i = 0; i < Count; ++i)
		{
			const float Seed = i * 2.399f;
			aPoints[i] = vec2(Background.x + Background.w * (0.5f + 0.47f * std::sin(Seed + Time * (0.025f + i * .0004f))), Background.y + Background.h * (0.5f + 0.46f * std::cos(Seed * 1.37f + Time * (0.02f + i * .0003f))));
			aDots[i] = IGraphics::CQuadItem(aPoints[i].x - 1.2f, aPoints[i].y - 1.2f, 2.4f, 2.4f);
		}
		for(int i = 0; i < Count; ++i)
			for(int j = i + 1; j < Count; ++j)
				if(distance(aPoints[i], aPoints[j]) < g_Config.m_BcGqBackgroundLineDistance)
					aLines[NumLines++] = IGraphics::CLineItem(aPoints[i], aPoints[j]);
		Ui()->ClipEnable(&Background);
		Graphics()->TextureClear();
		if(g_Config.m_BcGqBackgroundLines)
		{
			Graphics()->LinesBegin(); Graphics()->SetColor(ColorRGBA(.7f, .8f, 1.f, .12f)); Graphics()->LinesDraw(aLines, NumLines); Graphics()->LinesEnd();
		}
		if(g_Config.m_BcGqBackgroundTriangles)
		{
			Graphics()->QuadsBegin(); Graphics()->SetColor(ColorRGBA(.85f, .65f, 1.f, .25f)); Graphics()->QuadsDrawTL(aDots, Count); Graphics()->QuadsEnd();
		}
		// Rotating projected wire cubes.
		constexpr int MaxCubes = 10;
		constexpr int aEdges[12][2] = {{0,1},{1,3},{3,2},{2,0},{4,5},{5,7},{7,6},{6,4},{0,4},{1,5},{2,6},{3,7}};
		IGraphics::CLineItem aCubeLines[MaxCubes * 12]; int NumCubeLines = 0;
		for(int c = 0; c < g_Config.m_BcGqBackgroundCubes; ++c)
		{
			vec2 aV[8]; const float A = Time * (.12f + c * .01f) + c; const float S = (18.f + (c % 4) * 7.f) * g_Config.m_BcGqBackgroundCubeSize / 100.f;
			const vec2 Center(Background.x + Background.w * (.08f + .84f * ((c * 37) % 101) / 100.f), Background.y + Background.h * (.1f + .8f * ((c * 61) % 101) / 100.f));
			for(int v = 0; v < 8; ++v)
			{
				float X = (v & 1) ? 1.f : -1.f, Y = (v & 2) ? 1.f : -1.f, Z = (v & 4) ? 1.f : -1.f;
				const float RX = X * std::cos(A) - Z * std::sin(A); Z = X * std::sin(A) + Z * std::cos(A); X = RX;
				const float RY = Y * std::cos(A * .73f) - Z * std::sin(A * .73f); Z = Y * std::sin(A * .73f) + Z * std::cos(A * .73f); Y = RY;
				const float P = 2.5f / (3.5f - Z); aV[v] = Center + vec2(X, Y) * S * P;
			}
			for(const auto &Edge : aEdges) aCubeLines[NumCubeLines++] = IGraphics::CLineItem(aV[Edge[0]], aV[Edge[1]]);
		}
		Graphics()->LinesBegin(); Graphics()->SetColor(ColorRGBA(1.f, 1.f, 1.f, .16f)); Graphics()->LinesDraw(aCubeLines, NumCubeLines); Graphics()->LinesEnd();
		Ui()->ClipDisable();
	}

	vec2 ScrollOffset(0, 0); CScrollRegionParams Params; Params.m_ScrollUnit = 120.f; Params.m_Flags = CScrollRegionParams::FLAG_CONTENT_STATIC_WIDTH;
	m_SettingsScrollRegion.Begin(&MainView, &ScrollOffset, &Params); MainView.y += ScrollOffset.y;
	CUIRect Left, Right; MainView.VSplitMid(&Left, &Right, 14.f);
	const auto RowHeight = [](int Rows) { return Rows * BUTTON_HEIGHT + maximum(0, Rows - 1) * BUTTON_SPACING; };
	const auto Next = [](CUIRect &View, CUIRect &Row, bool &First) { if(!First) View.HSplitTop(BUTTON_SPACING, nullptr, &View); View.HSplitTop(BUTTON_HEIGHT, &Row, &View); First = false; };
	static CUi::SDropDownState s_Shotgun, s_Laser;
	s_Shotgun.m_SelectionPopupContext.m_pScrollRegion = &m_GqShotgunDropDownScrollRegion; s_Laser.m_SelectionPopupContext.m_pScrollRegion = &m_GqLaserDropDownScrollRegion;
	const char *apEffects[] = {Localize("Default"), Localize("Pulse Spiral"), Localize("Prismatic"), Localize("Crystal Glow"), Localize("Enchanted Glow")};
	RenderSettingsBlock(RowHeight(6), &Left, Localize("Weapon effects"), nullptr, nullptr, [&](CUIRect View) {
		CUIRect Row, Label, Select; bool First = true;
		Next(View, Row, First); Row.VSplitMid(&Label, &Select, MARGIN); Ui()->DoLabel(&Label, Localize("Shotgun"), FONT_SIZE, TEXTALIGN_ML); g_Config.m_BcGqShotgunEffect = Ui()->DoDropDown(&Select, g_Config.m_BcGqShotgunEffect, apEffects, std::size(apEffects), s_Shotgun);
		Next(View, Row, First); Ui()->DoScrollbarOption(&g_Config.m_BcGqShotgunThickness, &g_Config.m_BcGqShotgunThickness, &Row, Localize("Beam thickness"), 25, 300, &CUi::ms_LinearScrollbarScale, 0, "%");
		Next(View, Row, First); Ui()->DoScrollbarOption(&g_Config.m_BcGqShotgunGlowPower, &g_Config.m_BcGqShotgunGlowPower, &Row, Localize("Glow power"), 1, 100, &CUi::ms_LinearScrollbarScale, 0, "%");
		Next(View, Row, First); Row.VSplitMid(&Label, &Select, MARGIN); Ui()->DoLabel(&Label, Localize("Laser"), FONT_SIZE, TEXTALIGN_ML); g_Config.m_BcGqLaserEffect = Ui()->DoDropDown(&Select, g_Config.m_BcGqLaserEffect, apEffects, std::size(apEffects), s_Laser);
		Next(View, Row, First); Ui()->DoScrollbarOption(&g_Config.m_BcGqLaserThickness, &g_Config.m_BcGqLaserThickness, &Row, Localize("Beam thickness"), 25, 300, &CUi::ms_LinearScrollbarScale, 0, "%");
		Next(View, Row, First); Ui()->DoScrollbarOption(&g_Config.m_BcGqLaserGlowPower, &g_Config.m_BcGqLaserGlowPower, &Row, Localize("Glow power"), 1, 100, &CUi::ms_LinearScrollbarScale, 0, "%");
	});
	RenderSettingsBlock(RowHeight(2), &Left, Localize("Trajectory helper"), nullptr, nullptr, [&](CUIRect View) { CUIRect Row; bool First=true; Next(View,Row,First); if(GameClient()->m_Menus.DoButton_CheckBox(&g_Config.m_BcGqShowWeaponCollision, Localize("Show weapon trajectories"), g_Config.m_BcGqShowWeaponCollision, &Row)) g_Config.m_BcGqShowWeaponCollision ^= 1; Next(View,Row,First); Ui()->DoLabel(&Row, Localize("Uses the active hook-collision key"), 11.f, TEXTALIGN_ML); });
	RenderSettingsBlock(RowHeight(1), &Left, Localize("Tee Status"), nullptr, nullptr, [&](CUIRect View) { CUIRect Row; bool First=true; Next(View,Row,First); if(GameClient()->m_Menus.DoButton_CheckBox(&g_Config.m_BcGqShowActionStatusAboveTee, Localize("Hook / Hammer status above Tee"), g_Config.m_BcGqShowActionStatusAboveTee, &Row)) g_Config.m_BcGqShowActionStatusAboveTee ^= 1; });
	RenderSettingsBindsBlock(EBindOptionGroup::GQ, &Left, Localize("Dummy quick binds"));
	auto HudBlock = [&](CUIRect *pColumn, const char *pTitle, int *pEnabled, int *pX, int *pY, int *pSize) { RenderSettingsBlock(RowHeight(4), pColumn, pTitle, nullptr, nullptr, [&](CUIRect View) { CUIRect Row; bool First=true; Next(View,Row,First); if(GameClient()->m_Menus.DoButton_CheckBox(pEnabled, Localize("Enable widget"), *pEnabled, &Row)) *pEnabled ^= 1; Next(View,Row,First); Ui()->DoScrollbarOption(pX,pX,&Row,Localize("Horizontal position"),0,100,&CUi::ms_LinearScrollbarScale,0,"%"); Next(View,Row,First); Ui()->DoScrollbarOption(pY,pY,&Row,Localize("Vertical position"),0,100,&CUi::ms_LinearScrollbarScale,0,"%"); Next(View,Row,First); Ui()->DoScrollbarOption(pSize,pSize,&Row,Localize("Size"),20,300,&CUi::ms_LinearScrollbarScale,0,"%"); }); };
	HudBlock(&Right, "COPY / NO HOOK HUD", &g_Config.m_BcGqDummyCopyHud, &g_Config.m_BcGqDummyCopyHudX, &g_Config.m_BcGqDummyCopyHudY, &g_Config.m_BcGqDummyCopyHudSize);
	HudBlock(&Right, "TOWER HUD", &g_Config.m_BcGqTowerHud, &g_Config.m_BcGqTowerHudX, &g_Config.m_BcGqTowerHudY, &g_Config.m_BcGqTowerHudSize);
	RenderSettingsBlock(RowHeight(8), &Right, Localize("Page atmosphere"), nullptr, nullptr, [&](CUIRect View) { CUIRect Row; bool First=true; Next(View,Row,First); if(GameClient()->m_Menus.DoButton_CheckBox(&g_Config.m_BcGqAnimatedBackground,Localize("Animated background"),g_Config.m_BcGqAnimatedBackground,&Row)) g_Config.m_BcGqAnimatedBackground^=1; Next(View,Row,First); Ui()->DoScrollbarOption(&g_Config.m_BcGqBackgroundCubes,&g_Config.m_BcGqBackgroundCubes,&Row,Localize("Cube count"),1,10); Next(View,Row,First); Ui()->DoScrollbarOption(&g_Config.m_BcGqBackgroundCubeSize,&g_Config.m_BcGqBackgroundCubeSize,&Row,Localize("Cube size"),50,200); Next(View,Row,First); Ui()->DoScrollbarOption(&g_Config.m_BcGqBackgroundCubeSpeed,&g_Config.m_BcGqBackgroundCubeSpeed,&Row,Localize("Animation speed"),0,200); Next(View,Row,First); if(GameClient()->m_Menus.DoButton_CheckBox(&g_Config.m_BcGqBackgroundTriangles,Localize("Animated triangles"),g_Config.m_BcGqBackgroundTriangles,&Row))g_Config.m_BcGqBackgroundTriangles^=1; Next(View,Row,First); Ui()->DoScrollbarOption(&g_Config.m_BcGqBackgroundTriangleCount,&g_Config.m_BcGqBackgroundTriangleCount,&Row,Localize("Triangle count"),2,28); Next(View,Row,First); if(GameClient()->m_Menus.DoButton_CheckBox(&g_Config.m_BcGqBackgroundLines,Localize("Constellation lines"),g_Config.m_BcGqBackgroundLines,&Row))g_Config.m_BcGqBackgroundLines^=1; Next(View,Row,First); Ui()->DoScrollbarOption(&g_Config.m_BcGqBackgroundLineDistance,&g_Config.m_BcGqBackgroundLineDistance,&Row,Localize("Line distance"),60,240); });
	CUIRect Padding; (Left.y > Right.y ? Left : Right).HSplitTop(MARGIN, &Padding, nullptr); m_SettingsScrollRegion.AddRect(Padding); m_SettingsScrollRegion.End();
}
