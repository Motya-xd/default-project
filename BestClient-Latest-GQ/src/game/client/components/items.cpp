...
void CItems::RenderLaser(vec2 From, vec2 Pos, ColorRGBA OuterColor, ColorRGBA InnerColor, float TicksBody, float TicksHead, int Type) const
{
	...
	const int GqEffect = Type == LASERTYPE_SHOTGUN ? g_Config.m_BcGqShotgunEffect : Type == LASERTYPE_RIFLE ? g_Config.m_BcGqLaserEffect : 0;
	const bool CrystalLaser = !GameClient()->m_BestClient.IsComponentDisabled(CBestClient::COMPONENT_VISUALS_CRYSTAL_LASER) && (UseCrystalLaser(Type) || GqEffect == 3);
	...
	if(Len > 0)
	{
		...
		float Ia = 1 - a;
		const float GqThickness = (Type == LASERTYPE_SHOTGUN ? g_Config.m_BcGqShotgunThickness : g_Config.m_BcGqLaserThickness) / 100.f;
		if(GqEffect != 0)
		{
			const vec2 Perp(Dir.y, -Dir.x); const float Time = Client()->LocalTime();
			Graphics()->TextureClear(); Graphics()->QuadsBegin();
			auto Beam = [&](vec2 A, vec2 B, float Width, ColorRGBA Color) { const vec2 O=Perp*Width; const IGraphics::CFreeformItem Q(A-O,A+O,B-O,B+O); Graphics()->SetColor(Color); Graphics()->QuadsDrawFreeform(&Q,1); };
			if(GqEffect == 1)
			{
				for(int i=0;i<32;i++){float A=i/32.f,B=(i+1)/32.f; vec2 P0=mix(From,Pos,A)+Perp*std::sin(A*Len/10.f-Time*5.f)*7.f*Ia; vec2 P1=mix(From,Pos,B)+Perp*std::sin(B*Len/10.f-Time*5.f)*7.f*Ia; const vec2 D=normalize(P1-P0),O=vec2(D.y,-D.x)*GqThickness; const IGraphics::CFreeformItem Q(P0-O,P0+O,P1-O,P1+O); Graphics()->SetColor(InnerColor); Graphics()->QuadsDrawFreeform(&Q,1);}
			}
			else if(GqEffect == 2)
				for(int i=0;i<12;i++){float A=i/12.f,B=(i+1)/12.f; Beam(mix(From,Pos,A),mix(From,Pos,B),7.f*Ia*GqThickness,color_cast<ColorRGBA>(ColorHSLA(std::fmod(Time*.18f+A,1.f),1.f,.55f,InnerColor.a)));}
			else if(GqEffect == 3)
			{ Beam(From,Pos,15.f*Ia*GqThickness,OuterColor.WithMultipliedAlpha(.18f)); Beam(From,Pos,3.f*Ia*GqThickness,ColorRGBA(1,1,1,InnerColor.a*.8f)); }
			else
